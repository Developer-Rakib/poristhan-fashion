...
const Purchase = () => {
    ...
    const handleQuantity = (e) => {
        ...
        const minOrder = parseInt(minimumOrderQuanity)
        const AvailQuantity = parseInt(availableQuanity)
        if (quanity < minOrder) {
            setQuantityError(`You Have to Purchase Atlest ${minimumOrderQuanity} Products`)
            return
        }
        if (quanity > AvailQuantity) {
                    ...
                    <form onSubmit={handlePurchase} className='sm:pl-16 sm:w-auto w-11/12 mx-auto sm:mx-0 text-left'>
                        ...
                        <div className='flex justify-center mb-3 sm:mb-0'>
                            <button
                                ...
                                type='submit' style={{ fontFamily: 'Open Sans, sans-serif', letterSpacing: '2px' }} className={(quanity < minimumOrderQuanity || quanity > availableQuanity) ? `bg-gray-400 text-white rounded-full  px-6 text-sm sm:text-base sm:px-10 py-1.5 sm:py-2` : `hover:bg-white  bg-primary mr-2 transition hover:text-primary rounded-full text-white border-2 border-primary px-6 text-sm sm:text-base sm:px-10 py-1.5 sm:py-2`}>Purchase</button>

                            <label
                                onClick={() => setPartDetail(true)}
                                htmlFor="purchase-modal"
                                style={{ fontFamily: 'Open Sans, sans-serif', letterSpacing: '2px' }} className="hover:bg-primary transition hover:text-white rounded-full text-primary border-2 border-primary  px-6 text-sm sm:text-base sm:px-10 py-1.5 sm:py-2">Details</label>
                        </div>
                    </form>
                    {
